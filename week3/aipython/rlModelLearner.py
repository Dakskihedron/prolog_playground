    ...
    def select_action(self, reward, next_state):
        ...
        for update in range(self.updates_per_step):
            self.Q[st][act] = self.R[st][act]+self.discount*(
                sum(self.T[st][act][nst]/self.visits[st][act]*
                    max(self.Q[nst][nact] for nact in self.actions)
...
mbl1 = Model_based_reinforcement_learner(mon_env.name, mon_env.actions, 0.9, updates_per_step=10,method="model-based(10)")
# Simulate(mbl1,mon_env).go(100000).plot()
mbl2 = Model_based_reinforcement_learner(mon_env.name, mon_env.actions, 0.9, updates_per_step=1, method="model-based(1)")
# Simulate(mbl2,mon_env).go(100000).plot()
